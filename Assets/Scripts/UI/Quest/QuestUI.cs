...
public class QuestUI : MonoBehaviour
{
    ...
    public void AddQuestToUI(Quest _quest)
    {
        ...
        newQuest.transform.GetChild(0).GetChild(2).GetComponent<Button>().onClick.AddListener(() => _questExpander.SetExpander(_quest));
    ...
    public int GetQuestIndex(int questID)
    {
        for (int i = 0; i < parent.childCount; i++)
        {
            if (int.TryParse(parent.GetChild(i).name, out int index) && questID == index)
                return i;
        }
